...
interface RequestEventPayload<TSource extends HTMLElement> {
  source: TSource;
  handle: (fetch: Window["fetch"]) => Promise<void>;
}

interface RequestSendInit<TSource extends HTMLElement> {
  source: TSource;
  init: Parameters<Window["fetch"]>;
}

interface RequestEventInit<TSource extends HTMLElement> extends RequestSendInit<TSource> {
