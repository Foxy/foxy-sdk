 ...
 */
export type Context<TData extends Data = Data, TError = unknown> = {
  ...
  /** Form validation errors returned by the `validate` action. */
  errors: TError[];
};
 ...
 * @template TError Type of error returned by the `validate` action.
 */
export type State<TData extends Data = Data, TError = unknown> =
  | {
      value: 'fail';
  ...
  | {
      value: { busy: 'fetching' };
      context: Context<TData, TError> & { data: null; edits: null };
    }
  | {
      value: { busy: 'creating' };
      context: Context<TData, TError> & { data: null; edits: Partial<TData> };
    }
  | {
      value: { busy: 'updating' };
      context: Context<TData, TError> & { data: TData; edits: Partial<TData> };
    }
  | {
      value: { busy: 'deleting' };
      context: Context<TData, TError> & { data: TData };
    }
  | {
      value: 'idle';
      context: Context<TData, TError>;
  ...
  | {
      ...
      context: Context<TData, TError> & { data: TData; edits: null };
    }
  | {
  ...
  | {
      value: { idle: 'template' };
      context: Context<TData, TError> & { data: null };
    }
  | {
