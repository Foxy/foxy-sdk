...
This only works when the repo is checked out as the `src/sdk` submodule of the local docker env repo, which is what the container mounts. A standalone clone elsewhere builds fine but is not served.
